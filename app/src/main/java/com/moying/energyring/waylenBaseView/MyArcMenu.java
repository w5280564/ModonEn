...
public class MyArcMenu extends ViewGroup implements View.OnClickListener {
     ...
     * @param defStyle
     */
    public MyArcMenu(Context context, AttributeSet attrs, int defStyle) {

        super(context, attrs, defStyle);
        // dp convert to px
        mRadius = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, mRadius, getResources().getDisplayMetrics());
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,  R.styleable.MyArcMenu, defStyle, 0);

        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.MyArcMenu_arcPosition) {
            ...
            } else if (attr == R.styleable.MyArcMenu_arcRadius) {// dp convert to px
                mRadius = a.getDimensionPixelSize(attr, mRadius);

            } else if (attr == R.styleable.MyArcMenu_arcMargin) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            // mesure child  父视图不对子视图施加任何限制，子视图可以得到任意想要的大小；
//            getChildAt(i).measure(MeasureSpec.UNSPECIFIED,   MeasureSpec.UNSPECIFIED);

//            // 得到child的lp
//            MarginLayoutParams lp = (MarginLayoutParams) getChildAt(i).getLayoutParams();
//            // 当前子空间实际占据的宽度
//            int childWidth = getChildAt(i).getMeasuredWidth() + lp.leftMargin + lp.rightMargin;
//            // 当前子空间实际占据的高度
//            int childHeight = getChildAt(i).getMeasuredHeight() + lp.topMargin  + lp.bottomMargin;
            // 测量每一个child的宽和高,  未计算Margin实际占据空间 参考 FlowLayout
