...
/**
 * Created by waylen on 2018/3/19.
 */
...
//    private int translateX = cycle / 4;
    ...
    private UpdateLoadingListener updateListener;
    ...
    private void initPaint(Context context) {
        ...
        wavePaint.setColor(Color.parseColor("#ffd800"));

        circlePaint = new Paint();
//        circlePaint.setStrokeWidth(5);
//        circlePaint.setStyle(Paint.Style.STROKE);
        circlePaint.setAntiAlias(true);//设置抗锯齿
        circlePaint.setColor(Color.parseColor("#949393"));
...
//        Log.e("TAG", "startPoint.y:" + startPoint.y);
    ...
    private void drawProcessText(Canvas canvas) {
        ...
        if (progress >= 100) {
            if (!isupdate) {
                isupdate = true;
                updateListener.updateListener(progress);
            }
        }
...
//        progress = 0;
    ...
    public void setProgress(final int targetProcess, int duration) {
        if (progress > 100 || progress < 0)
            throw new RuntimeException(getClass().getName() + "请设置[0,100]之间的值");
        progressAnimator = ValueAnimator.ofInt(progress, targetProcess);
...
//        currentPlayTime = waveProgressAnim.getDuration();
//        currentPlayTime = waveProgressAnim.getCurrentPlayTime();
...
//        waveProgressAnim.setCurrentPlayTime(currentPlayTime);
//        progressAnimator.setStartTime(currentPlayTime);
    }


    public void setUpdateListener(UpdateLoadingListener listener) {
        this.updateListener = listener;
    }

    public interface UpdateLoadingListener {
        void updateListener(int progress);
    }
