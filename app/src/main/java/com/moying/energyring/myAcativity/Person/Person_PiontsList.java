...
import com.moying.energyring.Model.Person_Pionts_Model;
import com.moying.energyring.R;
import com.moying.energyring.StaticData.StaticData;
import com.moying.energyring.myAcativity.LoginRegister;
import com.moying.energyring.myAdapter.person_PiontsRanking_Adapter;
...
public class Person_PiontsList extends Activity implements XRecyclerView.LoadingListener {
    private XRecyclerView pk_xrecy;
    private TextView rank_Txt,rankCount_Txt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        initHead(Person_PiontsList.this);
        initData(Person_PiontsList.this);
    }

    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("Person_PiontsList"); //统计页面(仅有Activity的应用中SDK自动调用，不需要单独写。"SplashScreen"为页面名称，可自定义)
        MobclickAgent.onResume(this);          //统计时长
    }
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("Person_PiontsList"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证 onPageEnd 在onPause 之前调用,因为 onPause 中会保存信息。"SplashScreen"为页面名称，可自定义
...
//        rankCountTextsColor(28, 72, rankIngCount + "赞", rankCount_Txt);
    ...
    public void TextsColor(int size, int endsize, String alltext, TextView myTxt) {
        ...
        int padEndSize = (int) (Float.parseFloat(saveFile.getShareData("scale", Person_PiontsList.this)) * endsize);
...
//        styledText.setSpan(new ForegroundColorSpan(Color.parseColor("#f27b7b")),0,alltext.length()-1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);//修改颜色
    ...
    public void initData(Context context) {
        PageIndex = 1;
        pageSize = 10;
        ListData(context, saveFile.BaseUrl + saveFile.PersonRank_List_Url + "?PageIndex=" + PageIndex + "&PageSize=" + pageSize);
    }
    @Override
    public void onRefresh() {
        PageIndex = 1;
        pageSize = 10;
        ListData(Person_PiontsList.this, saveFile.BaseUrl + saveFile.PersonRank_List_Url + "?PageIndex=" + PageIndex + "&PageSize=" + pageSize);
    }
    ...
    person_PiontsRanking_Adapter mAdapter;
    ...
    public void initlist(final Context context) {
        ...
        mAdapter = new person_PiontsRanking_Adapter(context, baseModel, pionts_Model);
        pk_xrecy.setAdapter(mAdapter);
        mAdapter.setOnItemClickLitener(new person_PiontsRanking_Adapter.OnItemClickLitener() {
            @Override
...
//                startActivity(intent);
    ...
    private List<Person_Pionts_Model.DataBean.UserSortListBean> baseModel;
    Person_Pionts_Model pionts_Model;
        ...
        x.http().get(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String resultString) {
                if (resultString != null) {
                    ...
                    pionts_Model = new Gson().fromJson(resultString, Person_Pionts_Model.class);
                    if (pionts_Model.isIsSuccess() && !pionts_Model.getData().equals("[]")) {
                        baseModel.addAll(pionts_Model.getData().getUser_Sort_List());
                        if (PageIndex == 1) {
