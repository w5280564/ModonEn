...
//        if (computator.getMaxCount() == visibleViewport.top){
//            linePaintTab[position].setColor(Color.BLACK);
//        }else{
//            linePaintTab[position].setColor(Color.GREEN);
//        }
        if (scale == 0) {
            scale = 1;
        }
...
//                }
    ...
    private void drawAxisLines(Canvas canvas, Axis axis, int position) {
        ...
        if (axis.hasLines()) {
            int valueToDrawIndex = 0;
//            float middle=(rawValuesTab[position][1]-rawValuesTab[position][0])/2;//把线画到两个点的中间位置，默认画在点所在的位置
...
//                if (chart.getChartData().getAxisXTop() == )
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
            ...
            if (axis.hasTiltedLabels()) {
                ...
                canvas.drawText(labelBuffer, labelBuffer.length - charsNumber, charsNumber, labelX, labelY, labelPaintTab[position]);
//                canvas.drawText(labelBuffer,labelBuffer.length-charsNumber,labelX,labelY);
            }
        }
